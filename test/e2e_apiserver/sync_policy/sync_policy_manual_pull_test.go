...
var _ = Describe("SyncPolicy Manual Pull", Label("sync-policy", "git"), func() {
	It("should execute manual pull sync end-to-end", Label("SP0010"), func() {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Minute)
		defer cancel()

		api := tools.GetV1alpha1SyncPolicyApi()

		// 1. Create target project and registry
		project, err := tools.CreateProjectFixture(ctx, "e2e-pull-target")
		Expect(err).NotTo(HaveOccurred())
		defer project.Cleanup(ctx)

		registry, err := tools.CreateHuggingFaceRegistryFixture(ctx, "e2e-pull", "https://hf-mirror.com")
		Expect(err).NotTo(HaveOccurred())
		defer registry.Cleanup(ctx)
		Expect(registry.ID).To(BeNumerically(">", 0))

		// 2. Create Manual + Pull Base Policy
		name := fmt.Sprintf("e2e-manual-pull-%d", time.Now().UnixNano())
		req := v1alpha1.V1alpha1CreateSyncPolicyRequest{
			Name:        name,
			Description: "e2e manual pull test",
		...
		for _, job := range jobsResp.SyncJobs {
			...
			if *job.Status == v1alpha1.SUCCEEDED_V1alpha1SyncJobStatus {
				Expect(logResp.Log).To(ContainSubstring("job completed successfully"))
			}
		}

		// 8. Verify local model was created
		modelsApi := tools.GetV1alpha1ModelsApi()
		_, _, err = modelsApi.ModelsGetModel(ctx, project.Name, "gpt2")
		// Model may or may not exist depending on whether pull actually succeeded;
		// the important part is that the job was executed.
		if err == nil {
			By("local model gpt2 was created successfully")
		} else {
			By(fmt.Sprintf("local model gpt2 may not have been created (expected if remote unavailable): %v", err))
		}
	})
})
