...
func setupFixture(ctx context.Context) *protoFixture {
	GinkgoHelper()
	return setupFixtureWithType(ctx, v1alpha1project.PRIVATE_V1alpha1ProjectType)
}

func setupFixtureWithType(ctx context.Context, projectType v1alpha1project.V1alpha1ProjectType) *protoFixture {
	GinkgoHelper()
	...
	Expect(err).NotTo(HaveOccurred(), "create project")
	DeferCleanup(func() {
		cleanupCtx := context.Background()
