...
SDK requests include the current i18next language as `Accept-Language` (via `src/shared/utils/apiRequestHeaders.ts`). If an API response varies by language, mark the query with `meta.localeDependent: true` so `src/queryClient.ts` invalidates it when the language changes.

Keep the feature key prefix intact — do not put a shared locale key before the feature key, because feature-level invalidation such as `invalidateQueries({ queryKey: modelKeys.all })` relies on prefix matching.
...
- Do not add per-request global headers around generated SDK calls — use `apiRequestHeaders.ts`.
