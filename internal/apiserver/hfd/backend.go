...
func (b *Backend) Bind(
	...
	b.permissionHookFunc = middleware.NewRepoEnforcer(authzService)
...
func (b *Backend) initGitStorage() {
	...
	if err != nil {
		log.Fatalw("create xet token issuer failed", "error", err)
	}

	sharedMirror, err := mirror.NewMirror(
		// Must stay non-nil: PushToRemote silently no-ops without a destination func.
		mirror.WithMirrorDestinationFunc(b.mirrorDestination),
		mirror.WithMirrorRefFilterFunc(b.mirrorRefFilter),
		// No receive hooks: mirror pulls use remote-namespace repo names the hooks would misparse.
