...
func (config *Config) Validate() error {
	if !config.Debug && (len(config.APIServer.TokenSigningSecret) < 32 || config.APIServer.TokenSigningSecret == defaultTokenSigningSecret) {
		return fmt.Errorf("apiServer.tokenSigningSecret must be a non-default secret of at least 32 bytes outside debug mode")
	}
