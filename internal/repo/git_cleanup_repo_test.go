...
func TestCollectLFSDryRunListsWithoutDeleting(t *testing.T) {
	...
	if res.Orphaned[0].SizeBytes != lfsTestObjectSize {
		t.Fatalf("orphan size = %d, want %d", res.Orphaned[0].SizeBytes, lfsTestObjectSize)
	}
	if !lfsStored(t, xs, dead) {
		t.Fatal("dry run unlinked the dead object")
	}
	if !lfsStored(t, xs, live) {
		t.Fatal("dry run unlinked the live object")
	}
	if after := repo.LFSSize(ctx); after != before {
		t.Fatalf("LFSSize() after dry run = %d, want %d", after, before)
	}
}
...
func TestCollectLFSDryRunDoesNotReportPreviouslyUnlinkedGarbage(t *testing.T) {
	ctx := context.Background()
	...
	before := repo.LFSSize(ctx)
	result, err := repo.CollectLFS(ctx, true)
	if err != nil {
		t.Fatal(err)
	}
	if len(result.Orphaned) != 0 || result.ReclaimedBytes != 0 {
		t.Fatalf("dry run = %+v, want no candidates or reclaimed bytes", result)
	}
	if repo.LFSSize(ctx) != before || !lfsStored(t, store, live) {
		t.Fatal("dry run modified storage")
	}
	result, err = repo.CollectLFS(ctx, false)
	if err != nil || result.ReclaimedBytes <= 0 {
		t.Fatalf("execute = %+v, %v; want pending garbage reclaimed", result, err)
	}
}
