...
func (g *gitRepo) CollectLFS(ctx context.Context, dryRun bool) (*git.LFSCollectResult, error) {
	...
	for _, oid := range res.Unlinked {
		result.Orphaned = append(result.Orphaned, &git.OrphanedLFS{OID: oid, SizeBytes: sizes[oid]})
	}
	if err != nil || dryRun {
		return result, err
	}
	sweep, err := g.gc.SweepStep(ctx, hfdgc.Options{Grace: g.gcGrace, DryRun: dryRun})
	if sweep != nil {
