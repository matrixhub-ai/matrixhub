...
1. 点击 **创建**，在弹窗中完成同步规则配置。
    ...
    - 配置 **资源过滤**、**目标项目**、**触发方式**、**带宽限制** 和 **覆盖已有资源**。
    ...
    | 场景 | 参考配置 |
    |------|----------|
    | 从远端仓库拉取单个模型 | 同步规则：拉取<br />源仓库：`matrixhub-remote`<br />资源名称：`Qwen/Qwen3-0.6B`<br />资源类型：模型<br />目标项目：`demo`<br />触发方式：手动<br />带宽限制：`-1` |
    | 从远端仓库批量拉取模型 | 同步规则：拉取<br />源仓库：`matrixhub-remote`<br />资源名称：`Qwen/**`<br />资源类型：模型<br />目标项目：`demo`<br />触发方式：定时<br />带宽限制：`1024 Kbps` |
    | 推送本地模型到远端仓库 | 同步规则：推送<br />目标仓库：`matrixhub-remote`<br />资源名称：`demo/Qwen3-0.6B`<br />资源类型：模型<br />目标项目：`test-org`<br />触发方式：手动<br />带宽限制：`-1` |
...
| 触发方式 / Cron | 手动执行或按五段式 cron 定时执行。仅选择定时时需要填写 cron。 |
