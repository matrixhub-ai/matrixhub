...
1. 点击 **创建**，在弹窗中完成同步规则配置。
    ...
    | 将项目中的全部本地模型推送到远端仓库 | 同步规则：推送<br />目标仓库：`matrixhub-remote`<br />资源名称：`demo/**`<br />资源类型：模型<br />目标项目：`test-org`<br />触发模式：定时<br />Cron：`0 0 * * *`<br />带宽限制：`1024 Kbps` |
...
| 资源名称 | 要同步的模型路径，如 `Qwen/Qwen3-0.6B`；拉取和推送都可用 `*`、`**` 或 `Qwen/**` 在源仓库或本地 MatrixHub 中批量匹配。 |
| 资源类型 | 当前仅支持 **模型**。 |
