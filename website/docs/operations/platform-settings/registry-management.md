...
Registry Management is used to configure the platform's connection information for accessing external model sources (such as Hugging Face, ModelScope, or enterprise internal sources). The registry created here serves as an **upstream data source connector**, which will be selected as the **Source Registry** in **Remote Synchronization**.

## Prerequisites

- **Permissions:** Only **Platform Admins** can create and manage registry configurations.
...
1. Log in to MatrixHub with an admin account, go to **Platform Management** in the navigation (or **Platform Settings** under the **Admin** dropdown), and open the **Registry Management** page.

    ![Registry Overview](./images/repo-overview.jpg)

1. Click **Create**, select the provider in the popup, and fill in the registry configuration.
...
- The "registry" in "Registry Management" is not a specific model or dataset address, but a reusable upstream connection configuration.
